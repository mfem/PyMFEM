...
def run(order=3,
   ...
   # Input meshes.
   mesh_1 = mfem.Mesh(src_mesh_file, 1, 1, false)
   mesh_2 = mfem.Mesh(tar_mesh_file, 1, 1, false)
   ...
   if src_fieldtype < 0:
       func_source=mfem.GridFunction(mesh_1, src_sltn_file)
   ...
   if fieldtype == 0:
       ...
       asert False, "GridFunction type not supported."
   ...
   if fieldtype == 0 and order == mesh_poly_deg:
      ...
      for i in range(NE):
         ...
         pos=et.Transform(ir)
         vxyz0[i * nsp: i * nsp + nsp]=pos[:, 0]
         vxyz0[i * nsp + NE * nsp: i * nsp + NE * nsp + nsp]=pos[:, 1]
         if dim == 3:
             vxyz0[i * nsp + 2 * NE * nsp: i * \
                 nsp + 2 * NE * nsp + nsp]=pos[:, 2]
   ...
   interp_vals0=interp_vals0.GetDataArray()
   ...
   if fieldtype <= 1:
      if (fieldtype == 0 and order == mesh_poly_deg) or fieldtype == 1:
         ...
         elem_dof_vals=np.array(nsp * tar_ncomp, dtype = float)
...
if __name__ == "__main__":
    ...
    meshfile1 = expanduser(
        join(os.path.dirname(__file__), '..', 'data', args.mesh1))
    meshfile2 = expanduser(
        join(os.path.dirname(__file__), '..', 'data', args.mesh2))
