...
class SIntegrationRule(mfem.PyIntegrationRule):
    ...
    def SetElementinclSurfaceWeight(self, Element):
        if self.dim == 1:
            ...
            for ip in range(self.GetNPoints()):
                intp = self.IntPoint(ip)
                intp.weight = self.Weights[ip, Element] * self.SurfaceWeights[ip, Element]
...
class SurfaceLFIntegrator(mfem.PyLinearFormIntegrator):
    ...
    def AssembleRHSElementVect(self, el, Tr, elvect):
        ...
        for ip in range(self.SIntRule.GetNPoints()):
            ...
            mfem.add_vector(elvect, self.SIntRule.IntPoint(ip).weight * val, self.shape, elvect)
...
class SubdomainLFIntegrator(mfem.PyLinearFormIntegrator):
    ...
    def AssembleRHSElementVect(self, el, Tr, elvect):
        ...
        for ip in range(self.CIntRule.GetNPoints()):
            ...
            mfem.add_vector(elvect, self.CIntRule.IntPoint(ip).weight * val, self.shape, elvect)
...
def run(ref_levels=3, order=2):
   ...
   print("============================================")
   if itype != IntegrationType.Volumetric1D:
      print("Mesh size dx:                       " + "{:g}".format(3.2 / 2**ref_levels))
   else:
      print("Mesh size dx:                       " + "{:g}".format(25 / 2**ref_levels))
   ...
   print("============================================")
   print("Computed value of surface integral: " + "{:.2e}".format(surface.Sum()))
   ...
   if (itype == IntegrationType.Volumetric1D or
       ...
       print("--------------------------------------------")
       print( "Computed value of volume integral:  " + "{:.2e}".format(volume.Sum()))
       print( "True value of volume integral:      " + "{:.2e}".format(Volume()))
...
if __name__ == "__main__":
    ...
    parser = ArgParser(description='Ex38 (Cut-Volume and Cut-Surface Integration)')
