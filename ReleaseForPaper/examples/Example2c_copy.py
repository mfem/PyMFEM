...
from prob_envs.MultiObjectivePoisson import hpPoisson_copy
import numpy as np
import time
...
prob_config = {
    ...
    # tau schedule parameters and num iterations/batches
    'num_iterations'    : 50,
    'num_batches'       : nbatches,
    'tau_min'           : np.log2(5e-4), #np.log2(1e-2), # np.log2(1e-3), #np.log2(1e-4),
    'tau_max'           : np.log2(1e-2),  #np.log2(5e-2),
    'M_warm'            : 75, #50 number of batches in warming phase
    'M_anneal'          : 50,  #25, # number of batches per tau in annealing phase
    'N_anneal'          : 10,  #10,  # number of target errors to train on (not counting intitial target)
    ...
    for param in params_to_eval:
        ...
        while not done:
            ...
            if prob_config['optimization_type'] == 'dof_threshold' and done:
                break
            #new_row = pd.DataFrame([action[0], action[1]], columns = ['theta', 'rho'])
            #rlactions.append(new_row, ignore_index = True) 
        ...
        adf_vs_fixed_df = adf_vs_fixed_df.append(
            {'target': np.round(np.power(2,param),8), 'theta': np.round(float(action[0].item()),3), 'rho' : np.round(float(action[1].item()),3), 'steps': env.k, 'dofs': env.sum_of_dofs, 'error' : rlerrors[-1], 'why': quit_reason},
            ignore_index=True)
        
        # dftgt = pd.DataFrame({'rlactions':rlactions}) # replace this with rlactions as a dataframe
        ...
        rlactions.to_csv(filename, index=False)
        ...
        # define Boolean for whether or not to do expert policy
        expert_policy_eval = False;
        if prob_config['problem_type'] == 'lshaped' and expert_policy_eval:
            ## Enact AMR policies
            costs = []
            actions = []
...
if save_data:
    ...
    print("length of theta: {}".format(len(rlactions.iloc[:,0])))
    print("length of rho: {}".format(len(rlactions.iloc[:,1])))
    print("length of rldofs: {}".format(len(rldofs)))
    print("length of rlerrors: {}".format(len(rlerrors)))
