"""
    ...
    EXAMPLE 1-C: h-refinement policy for L-shaped domain problem with robust angles
...
from prob_envs.RobustPoisson import RobustAnglePoisson
...
prob_config = {
    ...
    'dof_threshold'         : 5e5,
    'error_threshold_lower' : 1e-3,
    'error_threshold_upper' : 1e-4,
}

## Change to minimum error problem
if not minimum_budget_problem:
    prob_config['optimization_type'] = 'dof_threshold'
    prob_config['angle_lower'] = np.pi * 0.25
    prob_config['angle_upper'] = np.pi * 0.75

## rllib parameters
config = ppo.DEFAULT_CONFIG.copy()
config['train_batch_size'] = 10 #100
config['sgd_minibatch_size'] = 5 # 50
config['rollout_fragment_length'] = 5 #20
...
register_env("my_env", lambda config : RobustAnglePoisson(**prob_config))
agent = ppo.PPOTrainer(env="my_env", config=config)
env = RobustAnglePoisson(**prob_config)
if train:
    ...
    checkpoint_path = "/Users/keith10/ray_results/PPO_my_env_2021-12-13_19-44-12gg4klhdt/checkpoint_000250/checkpoint-250"
...
ax1.legend()
plt.savefig('Example1a_fig1.pdf',format='pdf', bbox_inches='tight')
...
plt.tight_layout()
plt.savefig('Example1a_fig2.pdf',format='pdf', bbox_inches='tight')
...
plt.tight_layout()
plt.savefig('Example1a_fig3.pdf',format='pdf', bbox_inches='tight')
...
ax4.legend()
plt.savefig('Example1a_fig4.pdf',format='pdf', bbox_inches='tight')
...
ax5.legend()
plt.savefig('Example1a_fig5.pdf',format='pdf', bbox_inches='tight')
...
plt.savefig('Example1a_fig6.pdf',format='pdf', bbox_inches='tight')
