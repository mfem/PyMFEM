...
class GridFunction(mfem._ser.vector.Vector):
    ...
    def ComputeElementL2Error(self, ielem, exsol, irs=0):
        r"""ComputeElementL2Error(GridFunction self, int ielem, Coefficient exsol, mfem::IntegrationRule const *[] irs=0) -> double"""
        return _gridfunc.GridFunction_ComputeElementL2Error(self, ielem, exsol, irs)
    ComputeElementL2Error = _swig_new_instance_method(_gridfunc.GridFunction_ComputeElementL2Error)
...
def BoundingBox(face_patch, ufes, order, xmin, xmax, angle, midpoint, iface=-1, offsets=None):
    r"""BoundingBox(intArray face_patch, FiniteElementSpace ufes, int order, Vector xmin, Vector xmax, double & angle, Vector midpoint, int iface=-1, Vector offsets=None)"""
    return _gridfunc.BoundingBox(face_patch, ufes, order, xmin, xmax, angle, midpoint, iface, offsets)
BoundingBox = _gridfunc.BoundingBox

def LSZZErrorEstimator(blfi, u, error_estimates, subdomain_reconstruction=True, with_coeff=False, tichonov_coeff=0.0, sol_based=False, perfaces=None, xoffsets=None, yoffsets=None):
    r"""LSZZErrorEstimator(BilinearFormIntegrator blfi, GridFunction u, Vector error_estimates, bool subdomain_reconstruction=True, bool with_coeff=False, double tichonov_coeff=0.0, bool sol_based=False, intArray perfaces=None, doubleArray xoffsets=None, doubleArray yoffsets=None) -> double"""
    return _gridfunc.LSZZErrorEstimator(blfi, u, error_estimates, subdomain_reconstruction, with_coeff, tichonov_coeff, sol_based, perfaces, xoffsets, yoffsets)
